...
class ServerAPI{
    ...
    /**
     * 获取 Token 方法
     * @param $userId int   用户 Id，最大长度 32 字节。是用户在 App 中的唯一标识码，必须保证在同一个 App 内不重复，重复的用户 Id 将被当作是同一用户。
     * @param $name  string   用户名称，最大长度 128 字节。用来在 Push 推送时，或者客户端没有提供用户信息时，显示用户的名称。
     * @param $portraitUri string  用户头像 URI，最大长度 1024 字节。
     * @return json|xml
     */
